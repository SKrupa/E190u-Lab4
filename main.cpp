...
launch_cudaProcess(dim3 grid, dim3 block, int sbytes,
                   ...
                   int radius, float threshold, float highlight, int edges, uchar4 *oldData);
...
////////////////////////////////////////////////////////////////////////////////
void process(int width, int height, int radius, int edges)
{
    cudaArray *in_array;
    ...
    launch_cudaProcess(grid, block, sbytes,
                       ...
                       block.x+(2*radius), radius, 0.8f, 4.0f, edges, oldData);
...
{
    // run the Cuda kernel
    process(image_width, image_height, blur_radius, edge);
...
{
	uchar4 *oldDataC;
	int size = sizeof(uchar4)*512*512;
	oldDataC = (uchar4*)malloc(size);
	...
	cudaMalloc(&oldData, size);
	cudaMemcpy(oldData, oldDataC, size, cudaMemcpyHostToDevice);
	free(oldDataC);
